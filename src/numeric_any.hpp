...
private:
    ...
    details::numeric_types type{details::numeric_types::EMPTY};
...
#define FUNCTION_TO_RESTORE_VALUE(type_name, type_enum_name, enum_value)                                               \
    case details::numeric_types::type_enum_name: {                                                                     \
        ...
        if constexpr (::std::is_floating_point_v<type_name>) {                                                         \
            if (!details::is_normal_number(restore_value)) return ::std::nullopt;                                      \
...
constexpr bool numeric_any::can_safe_convert_to() const noexcept {
    ...
    if (type == details::numeric_types::BOOL) {
...
#define FUNCTIONS_OPERATION(name, op, op_name, op2, op2_name)                                                          \
    ...
    constexpr numeric_any& numeric_any::op2_name(const numeric_any& x) noexcept {                                      \
        ...
        } else if (type == details::numeric_types::DOUBLE || x.type == details::numeric_types::DOUBLE) {               \
            casting_##name<double>(unchecked_numeric_cast<double>(x));                                                 \
        } else if (type == details::numeric_types::FLOAT || x.type == details::numeric_types::FLOAT) {                 \
            casting_##name<float>(unchecked_numeric_cast<float>(x));                                                   \
        } else if (type == details::numeric_types::UNSIGNED_LONG_LONG ||                                               \
                   x.type == details::numeric_types::UNSIGNED_LONG_LONG) {                                             \
            casting_##name<unsigned long long>(unchecked_numeric_cast<unsigned long long>(x));                         \
        } else if (type == details::numeric_types::LONG_LONG || x.type == details::numeric_types::LONG_LONG) {         \
            casting_##name<long long>(unchecked_numeric_cast<long long>(x));                                           \
        } else if (type == details::numeric_types::UNSIGNED_LONG || x.type == details::numeric_types::UNSIGNED_LONG) { \
            casting_##name<unsigned long>(unchecked_numeric_cast<unsigned long>(x));                                   \
        } else if (type == details::numeric_types::LONG || x.type == details::numeric_types::LONG) {                   \
            casting_##name<long>(unchecked_numeric_cast<long>(x));                                                     \
        } else if (type == details::numeric_types::UNSIGNED_INT || x.type == details::numeric_types::UNSIGNED_INT) {   \
...
[[nodiscard]] constexpr ::std::partial_ordering numeric_any::operator<=>(const numeric_any& x) const noexcept {
    ...
    if (type == details::numeric_types::DOUBLE || x.type == details::numeric_types::DOUBLE)
        return unchecked_numeric_cast<double>(*this) <=> unchecked_numeric_cast<double>(x);
    if (type == details::numeric_types::FLOAT || x.type == details::numeric_types::FLOAT)
