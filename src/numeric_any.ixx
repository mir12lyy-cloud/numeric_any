...
public:
    ...
    [[nodiscard]] constexpr bool empty() const noexcept {
        return type_ == numeric_types::EMPTY;
    }
...
private:
    ...
    numeric_types type_{numeric_types::EMPTY};
...
export constexpr numeric_any bpow(const numeric_any& a, unsigned n) noexcept {
    return visit(a, [n](auto i) { return numeric_any{inner_bpow(i, n)}; }, [&a] { return a; });
}
...
export constexpr numeric_any abs(const numeric_any& x) noexcept {
    return visit(x, [](auto i) { return numeric_any{inner_abs(i)}; }, [&x] { return x; });
}
...
constexpr numeric_any::numeric_any(sign_unambiguous_arithmetic auto x) noexcept
    : width_{sizeof(decltype(x))}, type_{types<decltype(x)>()}, float_point_{::std::is_floating_point_v<decltype(x)>},
      is_unsigned_{::std::is_unsigned_v<decltype(x)>}, positive_{x >= decltype(x){}} {
...
#define FUNCTIONS_OPERATION(name, op, op_name, op2, op2_name)                                                          \
    ...
    constexpr numeric_any& numeric_any::op2_name(const numeric_any& x) noexcept {                                      \
        ...
        } else if (type_ == numeric_types::DOUBLE || x.type_ == numeric_types::DOUBLE) {                               \
            apply<double>(unchecked_numeric_cast<double>(x), [](auto x, auto y) { return x op y; });                   \
        } else if (type_ == numeric_types::FLOAT || x.type_ == numeric_types::FLOAT) {                                 \
            apply<float>(unchecked_numeric_cast<float>(x), [](auto x, auto y) { return x op y; });                     \
        } else if (type_ == numeric_types::UNSIGNED_LONG_LONG || x.type_ == numeric_types::UNSIGNED_LONG_LONG) {       \
            apply<unsigned long long>(unchecked_numeric_cast<unsigned long long>(x),                                   \
                                      [](auto x, auto y) { return x op y; });                                          \
        } else if (type_ == numeric_types::LONG_LONG || x.type_ == numeric_types::LONG_LONG) {                         \
            apply<long long>(unchecked_numeric_cast<long long>(x), [](auto x, auto y) { return x op y; });             \
        } else if (type_ == numeric_types::UNSIGNED_LONG || x.type_ == numeric_types::UNSIGNED_LONG) {                 \
            apply<unsigned long>(unchecked_numeric_cast<unsigned long>(x), [](auto x, auto y) { return x op y; });     \
        } else if (type_ == numeric_types::LONG || x.type_ == numeric_types::LONG) {                                   \
            apply<long>(unchecked_numeric_cast<long>(x), [](auto x, auto y) { return x op y; });                       \
        } else if (type_ == numeric_types::UNSIGNED_INT || x.type_ == numeric_types::UNSIGNED_INT) {                   \
